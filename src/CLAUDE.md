...
The formatter uses a hybrid approach: parse markdown/MDX into an AST (via remark), analyze it, then apply targeted line-based operations to the original text. This preserves formatting that AST round-tripping would destroy.
...
Plugins are remark-compatible transform functions applied during AST processing in `hybrid-formatter.ts`:
