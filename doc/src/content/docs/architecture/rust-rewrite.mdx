...
| napi-rs CI pipeline | Pending | Cross-platform binary generation (macOS/Linux/Windows) |
...
- napi-rs CI build pipeline (GitHub Actions for cross-platform `.node` binary generation)
