...
Since the crate is not yet published to crates.io, add it as a Git dependency pointing to the subdirectory:

```toml
[dependencies]
mdx-formatter-core = { git = "https://github.com/Takazudo/mdx-formatter", path = "crates/mdx-formatter-core" }
```
...
| `FormatterSettings` | `types` | Settings struct (via `types` module) |
| `FormatterOperation` | `types` | Operation enum (for advanced use) |

The `parser` and `types` modules are also public for consumers who need lower-level access to the AST parsing or operation types.
