...
The formatter uses remark-compatible plugins for specialized transformations. These run during AST processing in `src/mdx-formatter.ts`:
...
| File | Role |
| --- | --- |
| `src/mdx-formatter.ts` | Core formatter (`MdxFormatter`): parse AST, collect operations, apply |
