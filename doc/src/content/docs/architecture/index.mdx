...
parse: Phase 1 — Parse {
  source: Source text
  parser: remark/mdx parser
  ast: mdast AST
  source -> parser -> ast
}

analyze: Phase 2 — Analyze {
  walk: Walk AST
  ops: Collect line-based operations
  walk -> ops: "insert, replace,\nindent, fix"
}

apply: Phase 3 — Apply {
  sort: Sort operations\n(reverse order)
  lines: Apply to source lines
  normalize: Normalize empty lines
  sort -> lines -> normalize
}
