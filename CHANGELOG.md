...
- **CLI discovery now honors `.gitignore` by default** (issues [#140](https://github.com/Takazudo/mdx-formatter/issues/140) and [#123](https://github.com/Takazudo/mdx-formatter/issues/123)) — in the `1.3.0-next` prerelease line, the Node CLI adds the `ignore` package as a runtime dependency and automatically skips files ignored by Git during glob discovery. Explicit paths still bypass config `exclude` and automatic `.gitignore` rules, while `--ignore` and `--ignore-path` remain active. This is a behavior change: if a workflow relied on glob discovery checking Git-ignored generated files, pass `--no-gitignore` to opt out and restore the previous behavior. Quote glob operands when you want ignore rules to apply; shell-expanded filenames are treated as explicit paths.
